    ...
    def parse_shyfem_file(self, filename):
        ...
        try:
            ...
            while i < len(lines_content):
                ...
                # Parse polygons (starting with '2')  
                elif len(parts) >= 8 and parts[0] == '2':
                    ...
                    # Depth is the last value
                    depth = float(parts[4 + num_points]) if (4 + num_points) < len(parts) else 0.0
        ...
        for polygon_id, element_type, num_points, node_ids, depth in polygons:
            ...
            for node_id in node_ids:
                if node_id in points_dict:
                    x, y, _ = points_dict[node_id]
                    polygon_points.append(QgsPointXY(x, y))
            
            # For triangles (3 points), we need to close the ring
            if len(polygon_points) >= 3:
                # Close the polygon by adding the first point at the end
                if polygon_points[0] != polygon_points[-1]:
                    polygon_points.append(polygon_points[0])
                
                # Create polygon geometry - FIXED METHOD NAME
                polygon_geometry = QgsGeometry.fromPolygonXY([polygon_points])
                feature = QgsFeature()
                feature.setGeometry(polygon_geometry)
                feature.setAttributes([polygon_id, element_type, num_points, depth])
                features.append(feature)
    ...
    def import_shyfem_to_qgis(self, filename):
        ...
        try:
            ...
            if not points and not lines and not polygons:
                QMessageBox.warning(self.iface.mainWindow(), "Error", 
                                  "No valid points, lines, or polygons found in the GRD file")
                return False
            ...
            print(f"Imported: {len(points)} points, {len(lines)} lines, {len(polygons)} polygons")
            return True
